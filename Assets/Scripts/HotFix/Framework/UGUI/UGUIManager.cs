...
public class UGUIManager
{
    ...
    public UGUIManager() {
        Object.Instantiate(AssetManager.LoadAsset("UI/Prefabs/UIRoot")).SetZero();
    }
    ...
    public void OpenUIPanel<T>(object data = null) where T: UGUICtrl, new()
    {
        ...
        uiPanelCtrl[panelName].OpenPanel(data);//打开新面板
    }
    ...
    /// <typeparam name="T"></typeparam>
    public void OpenUI<T>(object data = null) where T: UGUICtrl, new()
    {
        //Debug.Log("直接打开ui");
        string panelName = typeof(T).ToString();
        ...
        uiWindowCtrl[panelName].OpenPanel(data);
    }
    ...
    public void OpenUIArchPos<T>(Vector3 v3,object data = null) where T: UGUICtrl, new()
    {
        string panelName = typeof(T).ToString();
        OpenUI<T>(data);
        uiWindowCtrl[panelName].mainView.GetComponent<RectTransform>().anchoredPosition = v3;
    }
    ...
    public void CloseUI<T>() where T: UGUICtrl, new()
    {
        ...
        if (uiWindowCtrl.ContainsKey(panelName))
        {
            uiWindowCtrl[panelName].ClosePanel();
        }
    }
