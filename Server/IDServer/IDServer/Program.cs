...
namespace IDServer
{
    internal class Program
    {
        ...
        [MessageHandler((ushort)MessageId.Chat)]
        private static void ChatMsg(ushort fromClientId, Message message) 
        {
            ushort playerId = message.GetUShort();
            string chat = message.GetString();
            Debug.Log(playerId + chat);
        }

        [MessageHandler((ushort)MessageId.Login)]
        private static void UserLogin(ushort fromClientId, Message message)
        {
            ushort playerId = message.GetUShort();
            string username = message.GetString();
            string password = message.GetString();
            Debug.Log("用户名" + username + "密码" + password);
            bool isOk = Game.GetComp<DBManager>().CreateUser(username, password);

            server.Send(Message.Create(MessageSendMode.reliable, MessageId.Login, 25).AddBool(isOk), fromClientId);
        }
